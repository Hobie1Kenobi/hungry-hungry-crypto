...
Status: **Phase 1 locked.** Orchestrator: ATLAS. Owner: Hobie Cunningham.
...
Real-time physics and input are **off-chain**. XRPL is identity, assets, receipts, and settlement only. Phase 1 performs **zero** XRPL transactions.

## Players

- 1–4 humans. Empty seats become AI.
- Phase 1: **1 local human** at seat 0. Seats 1–3 are Easy / Normal / Hungry fill. All seats emit the same `ChompInput`: `{ seat, down, clientTime }`.
...
- Local eat on overlap with a pellet. No server authority in Phase 1.
...
- `MatchResult.txHashes` stays `[]` in Phase 1.
...
Phase 1 stops here. Do not add Colyseus, wallets, or XRPL writes.
