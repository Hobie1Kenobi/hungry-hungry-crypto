...
- Practice clock stays frozen at 45.0s (`matchClockOrigin` 0, `timeLeft` ROUND_SECONDS) until a presented warm 3D frame: drawing buffer >= 8px, `gl.info.render.calls` >= 1, and two playing useFrames (first is pre-present). `startPractice` does not call `markMatchGo`. `applyMatchStart` still starts the online clock. `PRACTICE_GO_DUMP_T` still lands on that GO. Hitch clamp stays. Canvas wrap / WebGL clear are warm #eddcc6. drei `Preload all` compiles while lobby is up.
- Results camera pulls BACK (dist 14.6 / side 7.4 / elev 8.6) and looks at a pond-blended target. `setViewOffset` x=0 so the hero sits in open pond to the RIGHT of the left card. Playing path still `clearViewOffset()`. No per-frame neck-track on the playing camera. Visual necks still park short (0.94 latch).
- Results `h2` fits inside the card (15px, wrap, minmax grid). 3D hero is not shrunk to hide overflow.
