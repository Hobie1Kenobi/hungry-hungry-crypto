...
- Practice clock stays frozen at 45.0s (`matchClockOrigin` 0, `timeLeft` ROUND_SECONDS) until a presented warm 3D frame. The first playing `useFrame` still carries lobby render stats, so that frame is dropped. GO needs a real drawing buffer, `gl.info.render.calls` >= 12, triangles >= 2500, and two such playing presents (pond/beasts actually drew, not a warm clear-color hitch). `startPractice` does not call `markMatchGo`. `applyMatchStart` still starts the online clock. `PRACTICE_GO_DUMP_T` still lands on that GO. Hitch clamp stays. Canvas wrap / WebGL clear are warm #eddcc6. drei `Preload all` compiles while lobby is up.
