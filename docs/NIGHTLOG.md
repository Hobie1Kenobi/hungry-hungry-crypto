...
- Results camera pulls BACK (dist 10.2 / side 4.8 / elev 7.8) and looks at a pond-blended target, staying inside the studio walls. Cycle 13 `setViewOffset` pad=0.36 with x=pad parked the hero under the card; a longer orbit then put the lens outside the east/west walls (void). This cycle drops `setViewOffset`. Playing path still `clearViewOffset()`. No per-frame neck-track on the playing camera. Visual necks still park short (0.94 latch).
