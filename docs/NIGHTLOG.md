...
- Cheap ripple on eat and hopper dump. Tiny splash sprites at eat / land / dump. Hooked the existing juice bus: `notifyEat` and `notifyDump` now push `splashes[]`. `PelletChip` already called `notifySplash` on land. `Fx` draws rings + droplet sprites on that list. Pond draws surface ripple rings from the same list. No second juice bus. No fluid sim. No postprocessing water. No Rapier. No bloom.
